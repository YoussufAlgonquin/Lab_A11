...
> [!NOTE]
> **Deviation from the original instructions below:** the Algonquin student Azure tenant does not
> grant students the directory permission required to run `az ad app create`
> (`Insufficient privileges to complete the operation`). Azure AD App Registrations are a
> tenant/directory-level object and require the `Application Developer` (or higher) Entra ID role.
>
> Instead, we used **User-Assigned Managed Identities** (`az identity create` /
> `az identity federated-credential create`), which are plain Azure resources scoped to a resource
> group and only require the Contributor/Owner **Azure RBAC** role we already had — no directory
> permissions needed. Functionally this is equivalent for GitHub Actions OIDC: the identity's
> `clientId` is used exactly like an app's `appId` in the `AZURE_CLIENT_ID` secret, and federated
> credentials work the same way. See [`docs/3b-managed-identities.md`](3b-managed-identities.md)
> for the exact commands and resulting values used in this repo.
